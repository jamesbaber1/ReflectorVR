...
public class CrawlingEnemy : MovingEnemy
{
    ...
    void Start()
    {
        ...
        SpawnLazer();
        Vector3 goTo = player.transform.position;//new Vector3(radius * Mathf.Cos(Random.Range(-2 * Mathf.PI, 2 * Mathf.PI)), 0, radius * Mathf.Sin(Random.Range(-2 * Mathf.PI, 2 * Mathf.PI)));
